...
z_mid_occ <- array(dim = c(ndat, M, B, T)) # Expected and realized occurrence
x_mid_occ <- array(dim = c(ndat, B, T))

y_mid_occ<- array(NA, dim = c(ndat, M, B, J, T)) # Detection histories
...
for(d in 1:ndat){
  for(b in 1:B){
z_mid_occ[d,, b,1] <- rcat(M, psi[,1]) # Initial site state

# Initial block state, which is just a function of the states of the sites within each block
x_mid_occ[d,b,1] <- ifelse(sum(z_mid_occ[d,,b,1]==1) == M, 1, 
                      ifelse(sum(z_mid_occ[d,,b,1]==2) + sum(z_mid_occ[d,,b,1]==1) == M, 2,
                        ifelse(sum(z_mid_occ[d,,b,1]==3) + sum(z_mid_occ[d,,b,1]==1) == M, 3, 4) ) )
}}
...
for(d in 1:ndat){
  for(t in 1:(T-1)){
    for(b in 1:B){
      ...
      x_mid_occ[d,b,t+1] <- rcat(1, btpm[ ,x_mid_occ[d,b,t]])
      
      # generate latent site state
      for(i in 1:M){ # Loop over sites
        z_mid_occ[d, i,b,t+1] <- rcat(1, stpm[ ,z_mid_occ[d,i,b,t],x_mid_occ[d,b,t+1] ])
        }}}}
...
for(d in 1:ndat){
  for(b in 1:B){
    for(j in 1:M){
      for(k in 1:T){
        y_mid_occ[d,j, b, 1:7,k] <- rcat(7, rdm[,z_mid_occ[d,j,b,k]])
        }
      }
    }
  }
...
save(y_mid_occ, file="simdata_50set_50seas_8b_y_mid_occ.rda")
save(z_mid_occ, file="simdata_50set_50seas_8b_z_mid_occ.rda")
save(x_mid_occ, file="simdata_50set_50seas_8b_x_mid_occ.rda")
