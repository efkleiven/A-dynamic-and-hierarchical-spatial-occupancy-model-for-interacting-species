...
setwd("~/UiT/GitProjects/A-dynamic-and-hierarchical-spatial-occupancy-model-for-interacting-species/simulation_study")
#setwd("H:/UiT/GitProjects/A-dynamic-and-hierarchical-spatial-occupancy-model-for-interacting-species/simulation_study")
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          for(i in 1:nvisits) {
           ...
           # Draw a replicate data set under the fitted model
            ...
            yrep[i] ~ dcat( dpm[time_id[i], , z[site_id[i], block_id[i], time_id[i]]] + 0.01 )

            
            y2[(block_id[i]-1)*12 +   site_id[i], time_id[i], day_id[i]] <- y[i]
  ...
  for(q in 1:length(uniq_site1_t0)){
    z[uniq_site1_t0[q], uniq_block1_t0[q], uniq_time1_t0[q]+1] ~ dcat( stpm[( 1:nout ) , z[ uniq_site1_t0[q], uniq_block1_t0[q], uniq_time1_t0[q]], x[uniq_block1_t0[q], uniq_time1_t0[q]+1]] + 0.001)  # +0.01 to avoide giving the dcat a prob of 0 
...
yb <-y_hig_occ[1,,,,] # change name of imported object to fit with the rest of the code
...
# MCMC settings
ni <- 5000   ;   nt <- 10   ;   nb <- 1000   ;   nc <- 4    ;   na <- 1000
...
# Save model
setwd("./model_output")
save(out.gof, file="simdata_outgof_01.rda")
#~ End of script