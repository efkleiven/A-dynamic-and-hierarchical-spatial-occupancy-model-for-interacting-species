...
load("simdata_50set_50seas_8b_y_hig_occ.rda")     # data simulated under this model
#load("SimData.RData")
...
###################################
mod_4stpm_sim_hig_occ <- list() # make empty list to store models in
...
mod_4stpm_sim_hig_occ[[q]] <- jags(data, inits=inits, params, "mod.txt", n.chains = nc,
            n.thin = nt, n.iter = ni, n.burnin = nb, n.adapt=na, parallel = T)
}

# Save model
setwd("./model_output")
save(mod_4stpm_sim_hig_occ, file="mod_4stpm_sim_hig_occ.rda")
