...
z_low_occ <- array(dim = c(ndat, M, B, T)) # Expected and realized occurrence
x_low_occ <- array(dim = c(ndat, B, T))

y_low_occ<- array(NA, dim = c(ndat, M, B, J, T)) # Detection histories

psi[,1] <- c(0.6,0.2,0.15,0.05) # Initial site occupancy probability
...
for(d in 1:ndat){
  for(b in 1:B){
z_low_occ[d,, b,1] <- rcat(M, psi[,1]) # Initial site state

# Initial block state, which is just a function of the states of the sites within each block
x_low_occ[d,b,1] <- ifelse(sum(z_low_occ[d,,b,1]==1) == M, 1, 
                 ifelse(sum(z_low_occ[d,,b,1]==2) + sum(z_low_occ[d,,b,1]==1) == M, 2,
                        ifelse(sum(z_low_occ[d,,b,1]==3) + sum(z_low_occ[d,,b,1]==1) == M, 3, 4) ) )
}}
...
######################################################################
  ...
  stpm[d, b, t, 4, 1] <- gamA * gamB *(x_low_occ[d, b, t+1] == 4)                                                          #--|AB
   ...
   # A to ...
  stpm[d, b, t, 1, 2] <- epsA * (1-gamBA*((x_low_occ[d, b, t+1] == 3)+(x_low_occ[d, b, t+1] == 4)))       #--|U
  stpm[d, b, t, 2, 2] <- (1-epsA) * (1-gamBA*((x_low_occ[d, b, t+1] == 3)+(x_low_occ[d, b, t+1] == 4)))   #--|A
  stpm[d, b, t, 3, 2] <- epsA * gamBA *((x_low_occ[d, b, t+1] == 3)+(x_low_occ[d, b, t+1] == 4))           #--|B
  stpm[d, b, t, 4, 2] <- (1-epsA) * gamBA  *((x_low_occ[d, b, t+1] == 3)+(x_low_occ[d, b, t+1] == 4))     #--|AB
                       
  # B to ...
  stpm[d, b, t, 1, 3] <- (1-gamAB *((x_low_occ[d, b, t+1] == 2)+(x_low_occ[d, b, t+1] == 4))) * epsB       #--|U
  stpm[d, b, t, 2, 3] <- gamAB *((x_low_occ[d, b, t+1] == 2)+(x_low_occ[d, b, t+1] == 4)) * epsB           #--|A
  stpm[d, b, t, 3, 3] <- (1-gamAB *((x_low_occ[d, b, t+1] == 2)+(x_low_occ[d, b, t+1] == 4))) * (1-epsB)   #--|B
  stpm[d, b, t, 4, 3] <- gamAB *((x_low_occ[d, b, t+1] == 2)+(x_low_occ[d, b, t+1] == 4)) * (1-epsB)       #--|AB
...
for(i in 1:M){ # Loop over sites
    z_low_occ[d, i,b,t+1] <- rcat(1, stpm[d, b, t, ,z_low_occ[d,i,b,t]])
    }}}}
...
for(d in 1:ndat){
  for(b in 1:B){
    for(j in 1:M){
      for(k in 1:T){
        y_low_occ[d,j, b, 1:7,k] <- rcat(7, rdm[,z_low_occ[d,j,b,k]])
        }
      }
    }
  }
...
save(y_low_occ, file="simdata_50set_50seas_y_low_occ.rda")
save(z_low_occ, file="simdata_50set_50seas_z_low_occ.rda")
save(x_low_occ, file="simdata_50set_50seas_x_low_occ.rda")

table(y_low_occ)
table(z_low_occ)
table(x_low_occ) # x now has only observered states because predator and prey are very common
...
# do it on site level
z <- abind(z_low_occ[1,,1,],z_low_occ[1,,2,],z_low_occ[1,,3,],z_low_occ[1,,4,],along=1)
