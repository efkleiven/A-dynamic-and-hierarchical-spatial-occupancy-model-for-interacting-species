############################################################################################################################################
##  A dynamic and hierarchical spatial occupancy model for interacting species analyzing simulated data under a high detection scenario   ##
...
load("simdata_50set_50seas_8b_y_hig_det.rda")     # data simulated under this model
#load("SimData.RData")
...
###################################
mod_4stpm_sim_hig_det <- list() # make empty list to store models in
...
mod_4stpm_sim_hig_det[[q]] <- jags(data, inits=inits, params, "mod.txt", n.chains = nc,
