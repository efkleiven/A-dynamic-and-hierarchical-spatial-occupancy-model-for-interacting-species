...
###################################
mod_hidden_sim_mid_occ <- list() # make empty list to store models in
...
mod_hidden_sim_mid_occ[[q]] <- jags(data, inits=inits, params, "mac_dmsom_sdet.txt", n.chains = nc,
            n.thin = nt, n.iter = ni, n.burnin = nb, n.adapt=na, parallel = T)
}

# Save model
setwd("./model_output")
save(mod_hidden_sim_mid_occ, file="mod_hidden_sim_mid_occ.rda")
