    ...
    for(b in 1:B){
      ...
      # generate latent site state
        for(i in 1:M){ # Loop over sites
          z_hig_det[d, i,b,t+1] <- rcat(1, stpm[,z_hig_det[d,i,b,t],x_hig_det[d,b,t+1]])
        }         # end site loop
    }      # end block loop
  }    # end time loop
}  # end simulation loop
...
# save data
setwd("~/UiT/Manuskript/TeoreticalModelingOfSmallRodents&Mustelids/OccupancyModel/models//hidden_block_sim/data") # set directory where the simulations should be saved

save(y_hig_det, file="simdata_50set_50seas_8b_y_hig_det.rda")
save(z_hig_det, file="simdata_50set_50seas_8b_z_hig_det.rda")
save(x_hig_det, file="simdata_50set_50seas_8b_x_hig_det.rda")
