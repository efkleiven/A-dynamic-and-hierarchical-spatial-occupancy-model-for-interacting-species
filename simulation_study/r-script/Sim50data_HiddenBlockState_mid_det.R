...
z_mid_det <- array(dim = c(ndat, M, B, T)) # Expected and realized occurrence
x_mid_det <- array(dim = c(ndat, B, T))
...
table(y_mid_det)
table(z_mid_det)
table(x_mid_det) # x now has only observered states because predator and prey are very common
