##########################################################################################################################################
## A dynamic occupancy model for interacting species with two spatial scales analyzing simulated data under a low detection scenario   ##
...
mod_4stpm_sim_low_det <- list() # make empty list to store models in
...
for(q in 1:50){
  ...
  mod_4stpm_sim_low_det[[q]] <- jags(data, inits=inits, params, "mod.txt", n.chains = nc,
