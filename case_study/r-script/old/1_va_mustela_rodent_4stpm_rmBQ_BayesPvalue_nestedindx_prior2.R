...
cat("
    ...
    for(i in 1:2){    
      alphaA0[i] ~ dlogistic(0,1)
      alphaB0[i] ~ dlogistic(0,1)
    }

  # initial state parameters
    for(b in 1:nblock){
      for(i in 1:3){
        psi[b,i] ~ dbeta(4,4) # site
        }
...
#      for(j in 1:nsite){
#         for(day in 1:nsurvey) {  
#          y2[(b-1)*6+j,t, day] <- ifelse(yb[j,b,t,day]==1, 1,
#                                    ifelse(yb[j,b,t,day]==2, 2,
#                                      ifelse(yb[j,b,t,day]==3, 3,
#                                        ifelse(yb[j,b,t,day]==4, 4, 0))))
...
# seperate species
#  yrep_22 <- yrep==2       # select data point where status 2 (only A observed)
#  yrep_2 <- yrep_22*1      # make numeric

#  yrep_44 <- yrep==4       # select data point where status 4 ( A and B observed)
#  yrep_4 <- yrep_44*1      # make numeric

#  yrep_A <- yrep_2+yrep_4 

#  yrep_33 <- yrep==3       # select data point where status 2 (only A observed)
...
# dim yb, 1=site, 2=block, 3=primary occ, 4=secondary occ
has_data

# make data array wihtout NA's
y_long <- yb[!is.na(yb)]
...
data <-list(nseason = dim(yb)[3], nblock = dim(yb)[2], nsite = dim(yb)[1], nsurvey = dim(yb)[4], 
            nout=4, y = yb, season = season, e = 0.0001, nvisits=length(y_long), 
            time_id=time_id, site_id=site_id, block_id=block_id )
