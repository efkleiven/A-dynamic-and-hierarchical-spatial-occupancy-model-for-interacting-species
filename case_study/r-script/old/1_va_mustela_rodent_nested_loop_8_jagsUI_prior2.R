...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          for(i in 1:nvisits) {
           ...
           # Draw a replicate data set under the fitted model
            ...
            yb[site_id[i], block_id[i], time_id[i], day_id[i]] ~ dcat(c(0.836,0.15,0.012,0.002))
...
# MCMC settings
ni <- 4000   ;   nt <- 20   ;   nb <- 500  ;   nc <- 4    ;   na <- 1500
...
# Save model
setwd("./model_output")
save(out.gof, file="va_snowbed_mustela_rodent_outgof_jagui_nestedloop_ni10k_pri2_6.rda")
#~ End of script