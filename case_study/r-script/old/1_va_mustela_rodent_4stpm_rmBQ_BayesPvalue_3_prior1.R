...
# seperate species
 ...
 y_B <- y_3+y_4 
  ...
  for(t in 1:(nseason-1)){
      # ... for observed data
      ext_A[(b-1)*6+j,t] <- equals(zobs_A[(b-1)*6+j,(t+1)],0) * equals(zobs_A[(b-1)*6+j,t],1)
      nonext_A[(b-1)*6+j,t] <- equals(zobs_A[(b-1)*6+j,(t+1)],1) * equals(zobs_A[(b-1)*6+j,t],1)
      colo_A[(b-1)*6+j,t] <- equals(zobs_A[(b-1)*6+j,(t+1)],1) * equals(zobs_A[(b-1)*6+j,t],0)
      noncolo_A[(b-1)*6+j,t] <- equals(zobs_A[(b-1)*6+j,(t+1)],0) * equals(zobs_A[(b-1)*6+j,t],0)
      
      ext_B[(b-1)*6+j,t] <- equals(zobs_B[(b-1)*6+j,(t+1)],0) * equals(zobs_B[(b-1)*6+j,t],1)
      nonext_B[(b-1)*6+j,t] <- equals(zobs_B[(b-1)*6+j,(t+1)],1) * equals(zobs_B[(b-1)*6+j,t],1)
      colo_B[(b-1)*6+j,t] <- equals(zobs_B[(b-1)*6+j,(t+1)],1) * equals(zobs_B[(b-1)*6+j,t],0)
      noncolo_B[(b-1)*6+j,t] <- equals(zobs_B[(b-1)*6+j,(t+1)],0) * equals(zobs_B[(b-1)*6+j,t],0)
    
      # ... for replicated data
      extrep_A[(b-1)*6+j,t] <- equals(zobsrep_A[(b-1)*6+j,(t+1)],0) * equals(zobsrep_A[(b-1)*6+j,t],1)
      nonextrep_A[(b-1)*6+j,t] <- equals(zobsrep_A[(b-1)*6+j,(t+1)],1) * equals(zobsrep_A[(b-1)*6+j,t],1)
      colorep_A[(b-1)*6+j,t] <- equals(zobsrep_A[(b-1)*6+j,(t+1)],1) * equals(zobsrep_A[(b-1)*6+j,t],0)
      noncolorep_A[(b-1)*6+j,t] <- equals(zobsrep_A[(b-1)*6+j,(t+1)],0)*equals(zobsrep_A[(b-1)*6+j,t],0)
      
      extrep_B[(b-1)*6+j,t] <- equals(zobsrep_B[(b-1)*6+j,(t+1)],0) * equals(zobsrep_B[(b-1)*6+j,t],1)
      nonextrep_B[(b-1)*6+j,t] <- equals(zobsrep_B[(b-1)*6+j,(t+1)],1) * equals(zobsrep_B[(b-1)*6+j,t],1)
      colorep_B[(b-1)*6+j,t] <- equals(zobsrep_B[(b-1)*6+j,(t+1)],1) * equals(zobsrep_B[(b-1)*6+j,t],0)
      noncolorep_B[(b-1)*6+j,t] <- equals(zobsrep_B[(b-1)*6+j,(t+1)],0)*equals(zobsrep_B[(b-1)*6+j,t],0)
...
ptm <- proc.time()
out.gof <- jags(data, inits=inits, params, "mod_spatial_dynocc_bayp.txt", n.chains = nc,
                n.thin = nt, n.iter = ni, n.burnin = nb, n.adapt=na, parallel = T)
proc.time() - ptm

ptm2 <- proc.time()
out.gof <- run.jags("mod_spatial_dynocc_bayp.txt", data=data, inits=inits, monito=params,  n.chains = nc,
                   thin = nt, sample = ni, burnin = nb, adapt=na, method='parallel')
proc.time() - ptm2                
