...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          for(i in 1:nvisits) {
            ...
            y[i] ~ dcat( dpm[site_id[i], block_id[i], time_id[i], ( 1:nout ) , z[site_id[i], block_id[i], time_id[i]]] + 0.0005)  # +0.0005 to avoide giving the dcat a prob of 0 
    ...
    for(b in 1:nblock){
      ...
      for(j in 1:nsite){
        z[j, b, t+1] ~ dcat( stpm[( 1:nout ) , z[ j, b, t], x[b,t+1]] + 0.0005)  # +0.0005 to avoide giving the dcat a prob of 0 
...
out.gof2 <- jags(data, inits=inits, parameters.to.save=params, "mod_spatial_dynocc_bayp.txt", n.chains = nc,
                    n.thin = nt, n.iter = ni, n.burnin = nb, n.adapt=na, parallel = T)

# Save model
setwd("./model_output/revision2")
save(out.gof2, file="va_snowbed_mustela_rodent_gof_pri2_0005_2016_2021.rda")
#~ End of script