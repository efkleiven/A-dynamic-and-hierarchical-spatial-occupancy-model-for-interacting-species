...
# traceplots
par(mfrow=c(2,4))
traceplot(out.gof1, parameters = c("GamA","GamB","GamAB","GamBA", "EpsA", "EpsB", "EpsAB", "EpsBA"))
traceplot(out.gof1, parameters = c("gamA","gamB","gamAB","gamBA", "epsA", "epsB", "epsAB", "epsBA"))

# create df in a format suitable for ggplot
n <- 11400

dat <- data.frame(sims=unlist(out.gof1$sims.list[c(1:8,10:17)]),
...
dat2 <- data.frame(sims=unlist(out.gof1$mean[c(1:8,10:17)]),
...
ggplot(data=dat, aes(x=par, y=sims, fill="grey"))+
  ...
  stat_boxplot(data=dat, geom='errorbar', width=0.4, color="dark green", size=0.9)+
  geom_boxplot(data=dat, width=0.4, fill="white",alpha=0, color="dark green", size=0.9, outlier.shape = NA, coef = 0)+
...
# save plot
setwd("../plot")
ggsave("modperf_va_snowbed_mustela_rodent_site&block_rev2.png", width = 60, height = 30, units="cm")

###########################################################################################################################
##    Plot estimated detection probabilities

# making df in a suitable format for ggplot
dat3 <- data.frame(sims=c(unlist(out.gof1$sims.list[18][[1]][,1,1,c(1,15)]),
                          unlist(out.gof1$sims.list[19][[1]][,1,1,c(1,15)])),
                   par=c(rep("pA_S", times=n), rep("pA_W", times=n), rep("pB_S", times=n), rep("pB_W", times=n)) )

dat4 <- data.frame(sims=c(unlist(out.gof1$mean[18][[1]][1,1,c(1,15)]),
                          unlist(out.gof1$mean[19][[1]][1,1,c(1,15)])),
...
# save plot
ggsave("modperf_va_snowbed_mustela_rodent_p_rev2.png", width = 60, height = 20, units="cm")


## plot estimated initial occupancy probability

# make df with a suitable structure for ggplot
dat5 <- data.frame(sims=c(as.vector(out.gof1$sims.list$psi[,,1]),
                          as.vector(out.gof1$sims.list$psi[,,2]),
                          as.vector(out.gof1$sims.list$psi[,,3])),
...
# save the model
ggsave("modperf_va_snowbed_mustela_rodent_psi_rev2.png", width = 60, height = 30, units="cm")

####
str(out.gof1$mean[c(1,3,2,4,5,7,6,8)])
str(out.gof1$sd[c(1,3,2,4,5,7,6,8)])
str(out.gof1$q2.5[c(1,3,2,4,5,7,6,8)])
str(out.gof1$q97.5[c(1,3,2,4,5,7,6,8)])

str(out.gof1$mean[c(10,12,11,13,14,16,15,17)])
str(out.gof1$sd[c(10,12,11,13,14,16,15,17)])
str(out.gof1$q2.5[c(10,12,11,13,14,16,15,17)])
str(out.gof1$q97.5[c(10,12,11,13,14,16,15,17)])
##
...
quantile(out.gof1$sims.list$diff_gamA, probs=c(0.5,0.05,0.95)) 
quantile(out.gof1$sims.list$diff_gamB, probs=c(0.5,0.05,0.95)) 
quantile(out.gof1$sims.list$diff_epsA, probs=c(0.5,0.05,0.95)) 
quantile(out.gof1$sims.list$diff_epsB, probs=c(0.5,0.05,0.95))
